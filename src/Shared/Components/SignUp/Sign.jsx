...
function Sign() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await axios.post(`${apiurl()}/api/apisendotp`, {
        Email: data.email,
  ...
  const handleRegister = async () => {
    try {
      const response = await axios.put(`${apiurl()}/api/apiverifyotp`, {
        Email: data.email,
        OTP: otp,
      });
            ...
            {!otpStage ? (
              <>
                ...
                <div className='text-center mt-10'>
                  <button type='submit' className='p-3 w-full bg-[#E38734] hover:scale-105 duration-200 lg:text-xl text-lg text-white rounded-lg' onClick={handleSubmit}>
                    Send OTP
                  </button>
                </div>
              </>
