...
import { useAuth } from "../../Shared/Components/authenticated/AuthProvider";
...
export default function Header() {
  ...
  useEffect(() => {
    // Set local user from auth context
    setLocalUser(isAuthenticated ? localStorage.getItem("token") : null);
  }, [isAuthenticated]);
        ...
        <div className="max-w-[70rem] px-5 mx-auto">
          <div className="flex md:justify-between items-center">
            ...
            <div className="md:flex gap-5 hidden">
              ...
              <Link to={isAuthenticated ? '/' : '/login'} onClick={isAuthenticated ? logout : null}>
                <div className="relative">
                  {isAuthenticated ? <RiLogoutCircleRLine className="text-2xl text-[#E38734]"/> : 
                   <> <img
                    className="cursor-pointer h-7 opacity-85"
          ...
          <div className="space-y-4 mt-5 text-right text-base font-semibold">
            ...
            <div>
              <Link to="/login" onClick={() => { setOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
                <div className={`${isActive('/login') ? 'text-[#E38734]' : 'hover:text-[#E38734] cursor-pointer hover:scale-105 duration-200'}`}>{isAuthenticated ? 'Logout' : 'Login'}</div>
              </Link>
            </div>
          </div>
          {isOpen && (
